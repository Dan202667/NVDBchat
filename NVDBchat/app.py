...
def create_profile():
    ...
    if request.method == 'POST':
        username = request.form['username'].lstrip('@')  # убираем @ если случайно ввели
...
def search_user():
    ...
    target_participations = ChatParticipant.query.filter(
        ChatParticipant.chat_id.in_(current_user_chat_ids),
        ChatParticipant.user_id == user.id
    ).first()
    if target_participations:
        chat_id = target_participations.chat_id
    else:
